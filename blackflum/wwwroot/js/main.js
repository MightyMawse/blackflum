...
async function Login(role, user, sesName){
    var body = role == playerRole.PLAYER ? user : sesName;
    try{
        body = sesName + role.toString();
    }
    catch(error){ console.error("Login()" + error); }

    var loginResponse = SendRequest("POST", "/login", body);
